...
void WorldSocket::HandleAuthSessionCallback(std::shared_ptr<AuthSession> authSession, PreparedQueryResult result)
{
	...
	QueryResult VipInfo = LoginDatabase.PQuery("SELECT `Premium_Level` FROM `account_premium` WHERE `AccountId`='%u' AND `active`='1'", account.Id);
	authSession->m_viplevel= VipInfo ? VipInfo->Fetch()->GetUInt8() : 0;
    ...
    _worldSession = new WorldSession(account.Id, std::move(authSession->Account), shared_from_this(), account.Security,
        account.Expansion, authSession->m_viplevel, mutetime, account.Locale, account.Recruiter, account.IsRectuiter);
