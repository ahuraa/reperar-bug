...
void AdvancedEnchant(Player* player, Item* item, EnchantmentSlot slot, uint32 socketGem)
{
	if (!item)
	{
		ChatHandler(player->GetSession()).PSendSysMessage("Primero Debes equipar el Item!", LANG_UNIVERSAL);
		return;
	}
	...
	ChatHandler(player->GetSession()).PSendSysMessage("%s[%s]|r - Encantada con Exito!", color.c_str(), item->GetTemplate()->Name1.c_str());
}
...
class Npc_Enchants : public CreatureScript
{
public:
	Npc_Enchants() : CreatureScript("Npc_Enchants") { }
	...
	bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action)
	{
		...
		case 1:	// HEAD
		{
			...
			if (!item)
			{
				player->GetSession()->SendAreaTriggerMessage("Coloque su Cabeza por favor!");
			...
			player->ADD_GOSSIP_ITEM(10, "|TInterface\\PaperDollInfoFrame\\UI-GearManager-Undo:30:30:-14|tBack",					GOSSIP_SENDER_MAIN, 300);
			player->SEND_GOSSIP_MENU(1, creature->GetGUID());
			}
		...
		case 2: // SHOULDERS
		{
			...
			if (!item)
			{
				player->GetSession()->SendAreaTriggerMessage("Coloque su Hombrera por favor!");
				player->CastSpell(player, 27123);
				OnGossipHello(player, creature);
				return false;
			}
		...
		case 4: // GLOVES
		{
			...
			if (!item)
			{
				player->GetSession()->SendAreaTriggerMessage("Coloque sus Manos por favor!");
				player->CastSpell(player, 27123);
				OnGossipHello(player, creature);
				return false;
			}
		...
		case 5: // LEGS
		{
			...
			if (!item)
			{
				player->GetSession()->SendAreaTriggerMessage("Coloque sus Pantalones por favor!");
				player->CastSpell(player, 27123);
				OnGossipHello(player, creature);
				return false;
			}
		...
		case 6: // BRACERS
		{
			...
			if (!item)
			{
				player->GetSession()->SendAreaTriggerMessage("Coloque su Muñeca por favor!");
				player->CastSpell(player, 27123);
				OnGossipHello(player, creature);
				return false;
			}
		...
		case 7: // WAIST
		{
			...
			if (!item)
			{
				player->GetSession()->SendAreaTriggerMessage("Coloque su Cintura por favor!");
				player->CastSpell(player, 27123);
				OnGossipHello(player, creature);
				return false;
			}
		...
		case 8: // FEET
		{
			...
			if (!item)
			{
				player->GetSession()->SendAreaTriggerMessage("Coloque sus Botas por favor!");
				player->CastSpell(player, 27123);
				OnGossipHello(player, creature);
				return false;
			}
		...
		case 9: // CLOAK
		{
			...
			if (!item)
			{
				player->GetSession()->SendAreaTriggerMessage("Coloque su Capa por favor!");
				player->CastSpell(player, 27123);
				OnGossipHello(player, creature);
				return false;
			}
		...
		if (player->HasSkill(SKILL_ENGINEERING) && player->GetSkillValue(SKILL_ENGINEERING) == 450) // ENGINE
		 {
			...
			player->ADD_GOSSIP_ITEM(10, "|TInterface/PaperDollInfoFrame/UI-GearManager-Undo:30:30:-14|Back",						GOSSIP_SENDER_MAIN, 300);
			player->SEND_GOSSIP_MENU(1, creature->GetGUID());
			}
		...
		case 10: // RING 1
		{
			...
			if (!item)
			{
				player->GetSession()->SendAreaTriggerMessage("Coloque un Anillo por favor!");
				player->CastSpell(player, 27123);
				OnGossipHello(player, creature);
				return false;
			}
...

		{
			...
			if (!item)
			{
				player->GetSession()->SendNotification("Coloque Main-Hand por favor!");
				player->CastSpell(player, 27123);
				OnGossipHello(player, creature);
				return false;
			}
			...
			// ONE HAND
		{
			...
			if (!item)
			{
				player->GetSession()->SendNotification("One-Hand no esta equipada!", LANG_UNIVERSAL);
				player->CastSpell(player, 27123);
				OnGossipHello(player, creature);
				return false;
			}
		...
		
		{
			...
			if (!item)
			{
				player->GetSession()->SendNotification("Two-Hand no esta equipada!", LANG_UNIVERSAL);
				player->CastSpell(player, 27123);
				OnGossipHello(player, creature);
				return false;
			}
		...
		case 1210: // +110 Attack Power
		{
			...
			if (!item)
			{
				player->GetSession()->SendAreaTriggerMessage("Two-Hand no esta equipada.");
				player->CastSpell(player, 27123);
				OnGossipHello(player, creature);
				return false;
			}
			...
			// OFF HAND
		{
			...
			if (!item)
			{
				player->GetSession()->SendNotification("Off-Hand no esta equipada!", LANG_UNIVERSAL);
				player->CastSpell(player, 27123);
				OnGossipHello(player, creature);
				return false;
			}
...

		{
			...
			if (!item)
			{
				player->GetSession()->SendNotification("Shield no esta equipada!", LANG_UNIVERSAL);
				player->CastSpell(player, 27123);
				OnGossipHello(player, creature);
				return false;
			}
		...
		case 1320: // +81 Block & -50% Disarm 
		{
			...
			if (!item)
			{
				player->GetSession()->SendAreaTriggerMessage("Shield no esta equipado.");
				player->CastSpell(player, 27123);
				OnGossipHello(player, creature);
				return false;
			}
		...
		case 14: // RANGED
		{
			...
			if (!item)
			{
				player->GetSession()->SendNotification("Coloque su Ranged por favor!", LANG_UNIVERSAL);
				player->CastSpell(player, 27123);
				OnGossipHello(player, creature);
				return false;
			}
			...
			{
				player->GetSession()->SendAreaTriggerMessage("Coloque su Ranged por favor!");
				player->CastSpell(player, 27123);
				OnGossipHello(player, creature);
			}
		}
